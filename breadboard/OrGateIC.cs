    ...
    public OrGateIC(int row, Breadboard board, string label = "OR")
    {
        ...
        foreach (var (r, c) in pins)
        {
            if (board.IsOccupied(r, c))
                throw new InvalidOperationException($"Cannot place OR gate: position ({r},{c}) is already occupied.");
