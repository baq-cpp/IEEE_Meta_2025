public class AndGateIC
{
    ...
    public AndGateIC(int row, Breadboard board, string label = "AND")
    {
        this.label = label;
        pins = new (int, int)[14];
        ...
        for (int i = 0; i < 7; i++)
        {
            ...
            PinPositions[i + 1] = pins[i];         // Pin 1-7 left
            PinPositions[14 - i] = pins[13 - i];   // Pin 14-8 right
        }
        foreach (var (r, c) in pins)
        {
            if (board.IsOccupied(r, c))
                throw new InvalidOperationException($"Cannot place AND gate: position ({r},{c}) is already occupied.");
